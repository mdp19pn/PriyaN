                                                                                    ...
                                                                                    AbstractCellPopulation<ELEMENT_DIM,SPACE_DIM>& rCellPopulation)
{
     ...
     * their positions, because this method can be overloaded.
     */
...
PRINT_VARIABLE("force1") // does not print this message
    ...
    //assert(rest_length <= 1.0+1e-12); ///\todo #1884 Magic number: would "<= 1.0" do?
        PRINT_VARIABLE("force")
    //Checks if A and B are cells or ECM (particles).
    bool typeA = p_cell_A->template HasCellProperty<AbstractMammaryCellProperty>(); // GetNode(node_index)->IsParticle();
    bool typeB = p_cell_B->template HasCellProperty<AbstractMammaryCellProperty>();
    ...
    if (typeA != typeB) // For cell-ECM pair scale the spring constant by mCellECMSpringStiffness
    {
    	//Declare spring stiffness
    	double cell_ecm_spring_stiffness = mCellECMSpringStiffness;

    	if (dynamic_cast<MeshBasedCellPopulation<ELEMENT_DIM,SPACE_DIM>*>(&rCellPopulation))
    	{
    		return multiplication_factor * cell_ecm_spring_stiffness * unit_difference * overlap;
    	}
    	else
    	{
    		...
    		if (is_closer_than_rest_length) //overlap is negative
    		{
    			...
    			c_vector<double, SPACE_DIM> temp = multiplication_factor*cell_ecm_spring_stiffness * unit_difference * rest_length_final* log(1.0 + overlap/rest_length_final);
    			return temp;
    		}
    		else
    		{
    			double alpha = 5.0;
    			c_vector<double, SPACE_DIM> temp = multiplication_factor*cell_ecm_spring_stiffness * unit_difference * overlap * exp(-alpha * overlap/rest_length_final);
    			return temp;
    		}
        }
    }
    ...
    {
        if (typeA) // For cell-cell pair scale the spring constant by mCellCellSpringStiffness
        {
            //Declare spring stiffness
        ...
        else // For ECM-ECM pair scale the spring constant by mECMECMSpringStiffness
        {
            double ecm_ecm_spring_stiffness = mECMECMSpringStiffness;

            if (dynamic_cast<MeshBasedCellPopulation<ELEMENT_DIM,SPACE_DIM>*>(&rCellPopulation))
            {
                return multiplication_factor * ecm_ecm_spring_stiffness * unit_difference * overlap;
            }
            else
            {
                // A reasonably stable simple force law
                if (is_closer_than_rest_length) //overlap is negative
                {
                    //log(x+1) is undefined for x<=-1
                    assert(overlap > -rest_length_final);
                    c_vector<double, SPACE_DIM> temp = multiplication_factor*ecm_ecm_spring_stiffness * unit_difference * rest_length_final* log(1.0 + overlap/rest_length_final);
                    return temp;
                }
                else
                {
                    double alpha = 5.0;
                    c_vector<double, SPACE_DIM> temp = multiplication_factor*ecm_ecm_spring_stiffness * unit_difference * overlap * exp(-alpha * overlap/rest_length_final);
                    return temp;
                }
            }
        }
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double EpithelialLayerLinearSpringForce<ELEMENT_DIM,SPACE_DIM>::GetCellCellSpringStiffness()
{
    return mCellCellSpringStiffness;
}
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double EpithelialLayerLinearSpringForce<ELEMENT_DIM,SPACE_DIM>::GetCellECMSpringStiffness()
{
    return mCellECMSpringStiffness;
}
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double EpithelialLayerLinearSpringForce<ELEMENT_DIM,SPACE_DIM>::GetECMECMSpringStiffness()
{
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void EpithelialLayerLinearSpringForce<ELEMENT_DIM,SPACE_DIM>::SetCellCellSpringStiffness(double cellCellSpringStiffness)
{
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void EpithelialLayerLinearSpringForce<ELEMENT_DIM,SPACE_DIM>::SetCellECMSpringStiffness(double cellECMSpringStiffness)
{
    assert(cellECMSpringStiffness > 0.0);
    mCellECMSpringStiffness = cellECMSpringStiffness;
}

template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void EpithelialLayerLinearSpringForce<ELEMENT_DIM,SPACE_DIM>::SetECMECMSpringStiffness(double eCMECMSpringStiffness)
{
    assert(eCMECMSpringStiffness > 0.0);
