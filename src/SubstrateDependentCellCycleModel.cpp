...
void SubstrateDependentCellCycleModel::UpdateCellCyclePhase()
{
    ...
    if (mCurrentCellCyclePhase == G_ONE_PHASE)
    {
        ...
        if (cell_height < quiescent_height)
        {
             ...
             * CellPropertyRegistry::Instance() here when adding the mammary cell type, we would be creating
             * a new CellPropertyRegistry. In this case the mammary cell type's cell count would be incorrect.
             * We must therefore access the mammary cell type via the cell's CellPropertyCollection.
             */
            boost::shared_ptr<AbstractMammaryCellProperty> p_luminal =
                mpCell->rGetCellPropertyCollection().GetCellPropertyRegistry()->Get<LuminalCellProperty>();
            mpCell->AddCellProperty(p_luminal);

            boost::shared_ptr<AbstractMammaryCellProperty> p_myoepithelial =
