...
/* This completes the code for {{{MyoepithelialCellProperty}}}.  Note that usually this code would
 ...
 * causes all cells with the {{{MyoepithelialCellProperty}}} to move towards the origin. To do this we
...
public:
    ...
    void AddForceContribution(AbstractCellPopulation<2>& rCellPopulation)
    {
         ...
         * each node associated with cells with the {{{MyoepithelialCellProperty}}}, which is proportional (with constant {{{mStrength}}}) to the negative of the position. Causing
        ...
        for (AbstractCellPopulation<2>::Iterator cell_iter = rCellPopulation.Begin();
             ...
             ++cell_iter)
        {
            if (cell_iter->HasCellProperty<MyoepithelialCellProperty>())
            {
 ...
 */
class MyoepithelialCellProperty : public AbstractCellBasedTestSuite
{
public:
     ...
     */
    void TestMyoepithelialCellProperty()
    {
         ...
         * like {{{MyoepithelialCellProperty}}}, for which it makes sense for all cells
         ...
         * in the {{{MyoepithelialCellProperty}}} constructor.*/
        MAKE_PTR_ARGS(MyoepithelialCellProperty, p_property, (8));
        ...
        TS_ASSERT_EQUALS(p_property->IsType<MyoepithelialCellProperty>(), true);
        ...
        {
            AbstractCellProperty* const p_const_property = new MyoepithelialCellProperty(7);
            p_const_property->IncrementCellCount();

            TS_ASSERT_EQUALS(p_const_property->GetCellCount(), 1u);
            TS_ASSERT_EQUALS(dynamic_cast<MyoepithelialCellProperty*>(p_const_property)->GetColour(), 7u);
...

        {
            ...
            MyoepithelialCellProperty* p_real_property = dynamic_cast<MyoepithelialCellProperty*>(p_arch_property);
